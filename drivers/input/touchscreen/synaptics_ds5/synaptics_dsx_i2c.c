...
/* Debug mask value
 * usage: echo [debug_mask] > /sys/module/synaptics_dsx_i2c/parameters/debug_mask
*/
u32 touch_debug_mask = DEBUG_NONE;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			input_mt_sync(rmi4_data->input_dev);
#endif
			if (unlikely(touch_debug_mask & DEBUG_FINGER_INFO))
				dev_dbg(&rmi4_data->i2c_client->dev,
						"%s: Finger %d:\n"
						"status = 0x%02x\n"
						"x = %d\n"
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		if (unlikely(touch_debug_mask & DEBUG_BUTTON_INFO))
			dev_dbg(&rmi4_data->i2c_client->dev,
					"%s: Button %d (code %d) ->%d\n",
					__func__, button,
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (unlikely(touch_debug_mask & (DEBUG_FINGER_INFO | DEBUG_BUTTON_INFO)))
	dev_dbg(&rmi4_data->i2c_client->dev,
