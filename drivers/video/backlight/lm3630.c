...
static void lm3630_set_main_current_level(struct i2c_client *client, int level)
{
	...
	if (level != 0) {
		...
		/* LCD on/off, lm3630 off, from framework level setting 0 command, i2c error occurs */
...
#if defined(CONFIG_MACH_APQ8064_AWIFI) || defined(CONFIG_MACH_APQ8064_GVAR_CMCC)
